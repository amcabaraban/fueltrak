...
function setupAdminRoutes(app) {
    ...
    app.post('/api/sync-all-documents', authenticate, authorize('management'), async (req, res) => {
    ...
    app.get('/api/admin/optimize-database', authenticate, authorize('management'), async (req, res) => {
    ...
    app.get('/api/admin/create-so-table', authenticate, authorize('management'), async (req, res) => {
    ...
    app.get('/api/admin/create-login-attempts-table', authenticate, authorize('management'), async (req, res) => {
    ...
    app.post('/api/admin/verify-user', authenticate, authorize('management'), async (req, res) => {
    ...
    app.post('/api/admin/cleanup-loading', authenticate, authorize('management'), async (req, res) => {
    ...
    app.post('/api/admin/add-first-login-column', authenticate, authorize('management'), async (req, res) => {
    ...
    app.post('/api/admin/backup', authenticate, authorize('management'), async (req, res) => {
    ...
    app.get('/api/admin/backup-logs', authenticate, authorize('management'), async (req, res) => {
    ...
    app.post('/api/admin/restore', authenticate, authorize('management'), async (req, res) => {
        try { const {backup,mode='merge'}=req.body; if(!backup||typeof backup!=='object') return res.status(400).json({error:'Invalid backup data'}); let restored=0,errors=[]; for(const [table,records] of Object.entries(backup)){if(!Array.isArray(records)||!records.length) continue; if(!isSafeTableName(table)) { errors.push('Invalid table: '+table); continue; } try{if(mode==='replace') await pool.execute('DELETE FROM ??',[table]); for(const record of records){try{const columns=Object.keys(record).join(', ');const placeholders=Object.keys(record).map(()=>'?').join(', ');const values=Object.values(record);await pool.execute('INSERT IGNORE INTO ?? ('+columns+') VALUES ('+placeholders+')',[table,...values]);restored++;}catch(e){errors.push(table+': '+e.message);}}}catch(e){errors.push('Table '+table+': '+e.message);}} await logAudit(req.user.id,'RESTORE_BACKUP','system',0,{restored,errors:errors.length}); res.json({status:'success',message:'Restored '+restored+' records',errors:errors.slice(0,10),mode}); } catch(e) { res.status(400).json({error:e.message}); }
    });
}
