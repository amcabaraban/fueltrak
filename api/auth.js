...
app.post('/api/client/submit-atl', authenticate, authorize('client'), async (req, res) => {
  try {
    ...
    // If truck_id is provided, verify it exists
    if (truckId) {
    ...
    // If no truck_id but plate_no is provided, look up or create
    else if (plateNo) {
    ...
    if (!truckId) {
      return res.status(400).json({ error: 'Truck not found. Please verify the plate number.' });
    }
    
    // Check for existing pending ATL
    ...
    console.error('Submit ATL error:', error);
    res.status(400).json({ error: error.message });
  }
});
