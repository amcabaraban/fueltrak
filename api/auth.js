...
// ============ DATABASE CONNECTION ============
const pool = mysql.createPool({ host: process.env.DB_HOST, port: process.env.DB_PORT || 16287, user: process.env.DB_USER, password: process.env.DB_PASSWORD, database: process.env.DB_NAME, ssl: { rejectUnauthorized: false }, waitForConnections: true, connectionLimit: 25, queueLimit: 0, enableKeepAlive: true, keepAliveInitialDelay: 10000, connectTimeout: 10000, acquireTimeout: 15000, timeout: 60000, charset: 'utf8mb4' });
...
// ============================================================
// HEALTH, DISPATCH, CLIENT, TRUCK, SALES, CHAT, REPORTS, ADMIN
// ============================================================
...
app.delete('/api/trucks/delete/:id', authenticate, authorize('dispatcher','management'), async (req, res) => { try { const [m]=await pool.execute('SELECT plate_no FROM truck_masterlist WHERE id=?',[req.params.id]); if(m.length){const pn=m[0].plate_no;const [t]=await pool.execute('SELECT id FROM trucks WHERE plate_no=?',[pn]);if(t.length){await pool.execute('DELETE FROM truck_documents WHERE truck_id=?',[t[0].id]);await pool.execute('DELETE FROM trucks WHERE plate_no=?',[pn]);}await pool.execute('DELETE FROM truck_masterlist WHERE id=?',[req.params.id]);return res.json({status:'success',message:'Deleted'});} const [t]=await pool.execute('SELECT plate_no FROM trucks WHERE id=?',[req.params.id]);if(!t.length)return res.status(404).json({error:'Not found'});await pool.execute('DELETE FROM truck_documents WHERE truck_id=?',[req.params.id]);await pool.execute('DELETE FROM trucks WHERE id=?',[req.params.id]);await pool.execute('DELETE FROM truck_masterlist WHERE plate_no=?',[t[0].plate_no]);res.json({status:'success',message:'Deleted'}); } catch(e) { res.status(400).json({error:e.message}); } });
...
app.delete('/api/clients/:id', authenticate, authorize('dispatcher','management'), async (req, res) => { try { await pool.execute("DELETE FROM users WHERE id=? AND role='client'",[req.params.id]); res.json({status:'success',message:'Deleted'}); } catch(e) { res.status(400).json({error:e.message}); } });
...
app.delete('/api/users/:id', authenticate, authorize('dispatcher','management'), async (req, res) => { try { await pool.execute('DELETE FROM users WHERE id=?',[req.params.id]); res.json({status:'success',message:'Deleted'}); } catch(e) { res.status(400).json({error:e.message}); } });
